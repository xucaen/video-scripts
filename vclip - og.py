...
def process_clips(clips, min_clip_length):
    ...
    for row in clips:
        try:
            ...
            # Pulling the raw or string-formatted timestamps straight from the row configuration
            starttime = row.get("Start", "0.0")
            endtime = row.get("End", "0.0")
            
            # Sanitizing characters like ':' or '.' that might cause filename issues depending on your OS
            starttime_clean = str(starttime).replace(":", "-")
            endtime_clean = str(endtime).replace(":", "-")
