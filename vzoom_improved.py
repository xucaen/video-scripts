...
def main():
    ...
    # --- BOUNDING BOX ASPECT RATIO CALCULATIONS ---
    # Determine the maximum remaining space available from start positions (x, y)
    avail_w = target_w - args.x
    avail_h = target_h - args.y
    
    if avail_w <= 0 or avail_h <= 0:
        print(f"❌ ERROR: Start coordinates x={args.x}, y={args.y} are out of bounds for video dimensions {target_w}x{target_h}.")
        return

    # To maintain aspect ratio: calculated_w / calculated_h == target_w / target_h
    # Test maximizing width first
    w_candidate = avail_w
    h_candidate = int(round(w_candidate * target_h / target_w))
    
    if h_candidate <= avail_h:
        calculated_w = w_candidate
        calculated_h = h_candidate
    else:
        # If maximizing width exceeds height boundaries, maximize height instead
        calculated_h = avail_h
        calculated_w = int(round(calculated_h * target_w / target_h))

    # Bind calculated values back into the args namespace for seamless downstream access
    args.w = calculated_w
    args.h = calculated_h
    ...
    print(f"Calculated Box Width:     {args.w}")
    print(f"Calculated Box Height:    {args.h}")
    print(f"Calculated Box Ratio:     {args.w / args.h:.4f}")
    print("=" * 60)
